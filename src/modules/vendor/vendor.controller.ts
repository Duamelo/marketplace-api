...
export class VendorController {
    ...
    async addAvatar(@Req() request: RequestWithCustomer, @UploadedFile() file: Express.Multer.File) {
      return await  this.vendorService.addAvatar(request.user.id, file.buffer, file.originalname, file.mimetype);
    }
    ...
    async getVendorById(@Param('id') id: number){
        return await this.vendorService.findOneById(id);
    }

    @Get(':email')
    async getVendorByEmail(@Param('email') email: string){
        return await this.vendorService.findOneByEmail(email);
    }
